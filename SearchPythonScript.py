'''
Show Python Scripts where word is in text.
'''
...
if len(sys.argv) == 2:
    ...
    print("\n>> Searching for '{}' in Python Scripts.".format(word))
elif len(sys.argv) == 3:
    word = str(sys.argv[1]).upper().strip()
    word2 = str(sys.argv[2]).upper().strip()
    print("\n>> Searching for '{}' & '{}' in Python Scripts.".format(word, word2))
elif len(sys.argv) > 3:
    word = str(sys.argv[1]).upper().strip()
    word2 = str(sys.argv[2]).upper().strip()
    word3 = str(sys.argv[3]).upper().strip()
    print("\n>> Searching for '{}' & '{}' & '{}' in Python Scripts.".format(word, word2, word3))    
else:
    print("\n>> No search word given. Please provide search word as argument.\n")
    sys.exit()


# all Python files in U:/
file_list = []
...
print(">> Found {} Python Scripts.".format(len(file_list)))


print("\n>> FILES FOUND:")
# find word in Python scripts
list_lines = []
total_lines = 0
def FindWordPython(file_name):
