...
train <- subset(myts, end = length(myts)-12) 
test <- subset(myts, start = length(myts)-12+1) 
...
fc_naive <- naive(train, 12) #forecast
...
fc_snaive <- snaive(train, 12) #forecast
...
fc_mean <- meanf(train, h = 12) #forecast
...
e <- tsCV(train, forecastfunction = naive, h = 12) # Calculate errors
mse <- colMeans(e^2, na.rm = TRUE) # Compute the MSE values (remove missing values)
data.frame(h = 1:12, MSE = mse) %>% ggplot(aes(x = h, y = MSE)) + geom_point() # Plot MSE vs forecast horizon

e2 <- tsCV(train, forecastfunction = meanf, h = 12) # Calculate errors
mse2 <- colMeans(e2^2, na.rm = TRUE) # Compute the MSE values (remove missing values)
data.frame(h = 1:12, MSE = mse2) %>% ggplot(aes(x = h, y = MSE)) + geom_point() # Plot MSE vs forecast horizon


# METHOD 4 : Simple Exponential Smoothing (SES) = more recent observations get more weight
fc_ses <- ses(train, h = 12)
...
fc_holt <- holt(train, h = 12, PI = FALSE) # PI = prediction intervals
...
fc_holt_damped <- holt(train, damped = TRUE, h = 12, PI = FALSE) # PI = prediction intervals
...
fc_hw_add <- hw(train, h = 12, seasonal = "additive") # default : damped = FALSE
...
fc_hw_mul <- hw(train, h = 12, seasonal = "multiplicative") # default : damped = FALSE
...
fc_ets <- train %>% ets() %>% forecast(h=12) # forecast
