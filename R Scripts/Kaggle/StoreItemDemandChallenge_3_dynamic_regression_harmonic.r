...
# Trial 2 : output with Dynamic Regression - Harmonic #
...
forecastDemand <- function(item, store) {
  ...
  # time series : dynamic harmonic regression
  harmonics <- fourier(model_res, K = 10)
  fit_harmonic <- auto.arima(model_res, xreg = harmonics, seasonal = FALSE)
  newharmonics <- fourier(model_res, K = 10, h = 90)
  ...
  #autoplot(fc_harmonic) + autolayer(fitted(fc_harmonic), series = 'Fitted')
  #summary(fc_harmonic)
  #accuracy(fc_harmonic)

  ts_forecast <- xts(fc_harmonic$mean, order.by = as.Date(product_test$date))
...
write.csv( full_df[, c("id","sales")], paste(path_to_dir, "trial_3_dyn_reg_harmonic.csv", sep=""), row.names=FALSE)

full_df$sales_round <- round(full_df$sales)

write.csv( full_df[, c("id","sales_round")], paste(path_to_dir, "trial_3_dyn_reg_harmonic_rounded.csv", sep=""), row.names=FALSE)

# check
write.csv( full_df, paste(path_to_dir, "trial_3_dynamic_regression_harmonic_full.csv", sep=""), row.names=FALSE)
...
# TBATS             ME     RMSE     MAE      MPE     MAPE      MASE        ACF1
#Training set 0.3405577 5.464021 4.31406 58.40785 186.9117 0.7117319 0.006186398
...
# Dynamic Regression & Dynamic Harmonic Regression
#                   ME     RMSE      MAE      MPE     MAPE      MASE        ACF1
#Training set 0.3441112 6.308094 4.994187 -4.35646 19.79307 0.6851824 -0.01319412

# Dynamic Regression - Dynamic Harmonic Regression - Rounded
# Kaggle : 16.09883
