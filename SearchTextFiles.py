'''
Show Text files where word is in text.
'''
...
if len(sys.argv) == 2:
    ...
    print("\n>> Searching for '{}' in Text files.".format(word))
elif len(sys.argv) == 3:
    word = str(sys.argv[1]).upper().strip()
    word2 = str(sys.argv[2]).upper().strip()
    print("\n>> Searching for '{}' & '{}' in Text files.".format(word, word2))
elif len(sys.argv) > 3:
    word = str(sys.argv[1]).upper().strip()
    word2 = str(sys.argv[2]).upper().strip()
    word3 = str(sys.argv[3]).upper().strip()
    print("\n>> Searching for '{}' & '{}' & '{}' in Text files.".format(word, word2, word3))    
else:
...
print(">> Found {} Text files.".format(len(file_list)))
